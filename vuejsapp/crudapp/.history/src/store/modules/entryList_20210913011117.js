...
const actions = {
  ...
  async filterEntryList({ commit }, name, isActive) {
    if(name && isActive)
      const response = await axios.get(`http://localhost:8082/entries/filter?name=${name}&status=${isActive}`);
    else if(name)
      const response = await axios.get(`http://localhost:8082/entries/filter?name=${name}`);
    else if(isActive)
      const response = await axios.get(`http://localhost:8082/entries/filter?status=${isActive}`);
    else
      const response = await axios.get('http://localhost:8082/entries/');
